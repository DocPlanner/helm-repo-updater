...
import (
	"bytes"
	log "github.com/docplanner/helm-repo-updater/internal/app/logger"
	"text/template"
)
...
func TemplateCommitMessage(logger log.Logger, tpl *template.Template, appName string, changeList []ChangeEntry) string {
	...
	if err != nil {
		logger.ErrorWithContext("could not execute template for git commit message", err, log.LogContext{
			"application": appName,
		})
